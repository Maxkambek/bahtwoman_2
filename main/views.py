...
class MainTestCheck(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for i in range(1, len(ans) + 1):
            if i == 1:
                main = MainTest.objects.filter(id=2).first()
            elif i == 2:
                main = MainTest.objects.filter(id=2).first()
            elif i == 3:
                main = MainTest.objects.filter(id=5).first()
            else:
                main = MainTest.objects.filter(id=i + 2).first()
            if main and main.true_answer != ans[i - 1][f'{i}']:
                lst.append(i)
